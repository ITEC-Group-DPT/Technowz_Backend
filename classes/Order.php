    ...
    class Order{
        ...
        public function getOrderList($userID, $offset = 0, $limit = 5){
            ...
            foreach ($row as $item){
                ...
                if (!isset($orders[$orderID])) $orders[$orderID] = [];
                array_push($orders[$orderID], $obj);
        ...
        public function getItemList($orderID, $userID){
            $stmt = $this->conn->prepare("SELECT p.productID, p.name, i.img1, p.price, ordz.quantity, p.rating, p.sold, ordz.rating
