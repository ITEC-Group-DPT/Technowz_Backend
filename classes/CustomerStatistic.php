...
class CustomerStatistic
{
    ...
    public function getLeaderBoardData($limit = null, $time = 'month')
    {
        ...
        if ($time == 'month')
        {
            $stmt = $this->conn->prepare("SELECT ROW_NUMBER() OVER (ORDER BY sum(orders.totalPrice) DESC) AS rank, u.username, ifnull(sum(orders.totalPrice),0) AS purchasedAmount 
            FROM users u LEFT JOIN (orders inner Join orderstatus on orderstatus.orderID = orders.orderID and orderstatus.statusID = 4)  ON u.userID = orders.userID
            and MONTH(updateDate) = MONTH(NOW()) AND YEAR(updateDate) = YEAR(NOW())
            GROUP BY u.userID ORDER BY sum(orders.totalPrice) DESC {$str}");
        }
        else if ($time == 'year')
        {
            $stmt = $this->conn->prepare("SELECT ROW_NUMBER() OVER (ORDER BY sum(orders.totalPrice) DESC) AS rank, u.username,  ifnull(sum(orders.totalPrice),0)  AS purchasedAmount FROM users u LEFT JOIN (orders inner Join orderstatus on orderstatus.orderID = orders.orderID and orderstatus.statusID = 4)  ON u.userID = orders.userID
            and YEAR(updateDate) = YEAR(NOW())
            GROUP BY u.userID ORDER BY sum(orders.totalPrice) DESC  {$str}");
        }
        else if ($time == 'day')
        {
            $stmt = $this->conn->prepare("SELECT ROW_NUMBER() OVER (ORDER BY sum(orders.totalPrice) DESC) AS rank, u.username, ifnull(sum(orders.totalPrice),0)  AS purchasedAmount FROM users u LEFT JOIN (orders inner Join orderstatus on orderstatus.orderID = orders.orderID and orderstatus.statusID = 4)  ON u.userID = orders.userID
            and DATE(updateDate) = DATE(NOW())
            GROUP BY u.userID ORDER BY sum(orders.totalPrice) DESC  {$str}");
        }
        $stmt->execute();
        $result = $stmt->get_result();
