...
class Statistic
{
    ...
    {
        $stmt = $this->conn->prepare("SELECT count(orders.orderID) count FROM orders , orderstatus where orderstatus.orderID = orders.orderID and orderstatus.statusID = 4 and orderstatus.updateDate >= {$currentInterval}");
        $stmt->execute();
    ...
    {
        $stmt = $this->conn->prepare("SELECT SUM(orderdetails.quantity) count from orders, orderdetails , orderstatus where orderstatus.orderID = orders.orderID and orderstatus.statusID = 4 and orders.orderID = orderdetails.orderID and orderstatus.updateDate >= {$currentInterval}");
        $stmt->execute();
        $result = $stmt->get_result();
        $row = $result->fetch_all(MYSQLI_ASSOC);
        return $row;
    ...
    public function getTopRevenue($currentInterval)
    {
        ...
        FROM products LEFT JOIN (orderdetails inner JOIN (orders inner Join orderstatus on orderstatus.orderID = orders.orderID and orderstatus.statusID = 4)  on  orders.orderID = orderdetails.orderID and orderstatus.updateDate >= {$currentInterval}) on products.productID = orderdetails.productID GROUP by products.productID, products.name, products.price ORDER by revenue DESC LIMIT 5;");
        $stmt->execute();
        $result = $stmt->get_result();
        $row = $result->fetch_all(MYSQLI_ASSOC);
        return $row;
    ...
    {
        $stmt = $this->conn->prepare("SELECT products.productID,(SELECT productimage.img1 FROM productimage where productimage.productID = products.productID) productimg, products.name, products.price, ifnull(sum(orderdetails.quantity),0) unit, IFNULL(((sum(orderdetails.quantity)) - (SELECT sum(od.quantity) FROM orderdetails od, orders , orderstatus where orderstatus.orderID = orders.orderID and orderstatus.statusID = 4 and  orders.orderID = od.orderID and orderstatus.updateDate >= {$previousInterval} and orderstatus.updateDate < {$currentInterval} and od.productID = orderdetails.productID)) / (SELECT  sum(od.quantity) FROM orderdetails od, orders , orderstatus where orderstatus.orderID = orders.orderID and orderstatus.statusID = 4 and  orders.orderID = od.orderID and orderstatus.updateDate >= {$previousInterval} and orderstatus.updateDate < {$currentInterval} and od.productID = orderdetails.productID) * 100,100) as up 
    ...
    public function getMostProfitableCate($currentInterval, $previousInterval)
    {
        $stmt = $this->conn->prepare("SELECT p.type, IFNULL(total,0) total, dense_rank() OVER ( ORDER BY total DESC ) AS rank, ((
            select DISTINCT(rank) from (
                SELECT p.type, IFNULL(total,0) total, dense_rank() OVER ( ORDER BY total DESC ) AS rank FROM products p LEFT JOIN (SELECT products.type type, sum(products.price) total FROM orders, orderdetails, products , orderstatus where orderstatus.orderID = orders.orderID and orderstatus.statusID = 4 and  orderstatus.updateDate >= {$previousInterval} and orderstatus.updateDate < {$currentInterval} and orders.orderID = orderdetails.orderID and products.productID = orderdetails.productID GROUP by products.type ORDER BY `total` DESC) a on p.type = a.type) pa where pa.type = p.type) - dense_rank() OVER ( ORDER BY total DESC )) as up     
        FROM products p LEFT JOIN (SELECT products.type type, sum(products.price) total FROM orders, orderdetails, products , orderstatus where orderstatus.orderID = orders.orderID and orderstatus.statusID = 4 and orderstatus.updateDate >= {$currentInterval} and orders.orderID = orderdetails.orderID and products.productID = orderdetails.productID GROUP by products.type ORDER BY `total` DESC) a on p.type = a.type GROUP BY p.type ORDER BY  rank limit 5; ");
        $stmt->execute();
        $result = $stmt->get_result();
        $row = $result->fetch_all(MYSQLI_ASSOC);
        return $row;
